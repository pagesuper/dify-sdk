...
var text_decoder_1 = tslib_1.__importDefault(require("../utils/text-decoder"));
...
/** 检测是否为浏览器环境 */
var isBrowser = typeof window !== 'undefined' && typeof window.XMLHttpRequest !== 'undefined';
...
var DifyClient = /** @class */ (function () {
            ...
            return tslib_1.__generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 6:
                        ...
                        // 将Buffer转换为字符串
                        if (value !== undefined) {
                            buffer += text_decoder_1.default.decode(value, { stream: true });
                        }
    ...
    DifyClient.prototype.handleStreamWithXHR = function (params) {
        ...
        // Node.js环境使用http/https模块
            ...
            return new Promise(function (resolve, reject) {
                var url = new url_1.URL("".concat(_this.config.baseUrl, "/v1/chat-messages"));
                var protocol = url.protocol === 'https:' ? https : http;
                var body = JSON.stringify(tslib_1.__assign({ inputs: {} }, params));
                var headers = tslib_1.__assign({ Authorization: "Bearer ".concat(_this.config.apiKey), 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) }, _this.config.defaultHeaders);
                var options = {
                ...
                var req = protocol.request(options, function (res) {
                    ...
                    if (res.statusCode && res.statusCode >= 400) {
                        reject(new Error("Request failed: ".concat(res.statusCode, " ").concat(res.statusMessage)));
                        return;
                    }
                    // 处理流式响应
                    res.on('data', function (chunk) {
                        ...
                        while (true) {
                            ...
                            if (chunkData.trim().startsWith('data:')) {
                                try {
                                    var parsedChunk = JSON.parse(chunkData.replace(/^data: /, ''));
                                    if (typeof params.chunkCompletionCallback === 'function') {
                ...
                req.on('error', function (error) {
                    reject(new Error("Network error: ".concat(error.message)));
                });
                // 发送请求体
                req.write(body);
                req.end();
    ...
    DifyClient.prototype.handleWorkflowStream = function (params, url) {
        ...
        // Node.js环境使用http/https模块
            ...
            return new Promise(function (resolve, reject) {
                var requestUrl = new url_1.URL(url);
                var protocol = requestUrl.protocol === 'https:' ? https : http;
                ...
                var req = protocol.request(options, function (res) {
                    if (res.statusCode && res.statusCode >= 400) {
                        reject(new Error("Workflow failed: ".concat(res.statusCode, " ").concat(res.statusMessage)));
                        return;
                    }
                    res.on('data', function (chunk) {
                        buffer += chunk.toString();
            ...
            return tslib_1.__generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 5:
                        ...
                        buffer_1 += text_decoder_1.default.decode(value);
                        processBuffer();
                        return [3 /*break*/, 4];
